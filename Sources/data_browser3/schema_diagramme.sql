...
begin
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'SPRINGY_DIAGRAMS';
	if v_count = 0 then 
		v_stat := q'[
		CREATE TABLE SPRINGY_DIAGRAMS 
		   (ID NUMBER DEFAULT ON NULL SPRINGY_DIAGRAMS_SEQ.NEXTVAL NOT NULL ENABLE, 
			...
			CREATED_AT TIMESTAMP (6) WITH LOCAL TIME ZONE DEFAULT ON NULL LOCALTIMESTAMP NOT NULL ENABLE, 
			CREATED_BY VARCHAR2(32 CHAR) DEFAULT ON NULL NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), SYS_CONTEXT('USERENV','SESSION_USER')) NOT NULL ENABLE, 
			LAST_MODIFIED_AT TIMESTAMP (6) WITH LOCAL TIME ZONE DEFAULT ON NULL LOCALTIMESTAMP NOT NULL ENABLE, 
			LAST_MODIFIED_BY VARCHAR2(32 CHAR) DEFAULT ON NULL NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), SYS_CONTEXT('USERENV','SESSION_USER')) NOT NULL ENABLE, 
...
begin
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'DIAGRAM_SHAPES';
	if v_count = 0 then 
		v_stat := q'[
		CREATE TABLE DIAGRAM_SHAPES 
		   (	ID NUMBER DEFAULT ON NULL DIAG_SHAPES_SEQ.NEXTVAL NOT NULL ENABLE, 
			DESCRIPTION VARCHAR2(512) NOT NULL ENABLE, 
...
begin
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'DIAGRAM_COLORS';
	if v_count = 0 then 
		v_stat := q'[
		CREATE TABLE DIAGRAM_COLORS 
		   ( ID NUMBER DEFAULT ON NULL DIAG_COLORS_SEQ.NEXTVAL NOT NULL ENABLE, 
...
begin
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'DIAGRAM_NODES';
	if v_count = 0 then 
		v_stat := q'[
		CREATE TABLE DIAGRAM_NODES 
		   (	ID NUMBER DEFAULT ON NULL DIAG_NODES_SEQ.NEXTVAL NOT NULL ENABLE, 
			SPRINGY_DIAGRAMS_ID NUMBER NOT NULL ENABLE, 
			DESCRIPTION VARCHAR2(512) NOT NULL ENABLE, 
			ACTIVE VARCHAR2(1) DEFAULT ON NULL 'Y' NOT NULL ENABLE, 
			CREATED_AT TIMESTAMP (6) WITH LOCAL TIME ZONE DEFAULT ON NULL LOCALTIMESTAMP NOT NULL ENABLE, 
			CREATED_BY VARCHAR2(32 CHAR) DEFAULT ON NULL NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), SYS_CONTEXT('USERENV','SESSION_USER')) NOT NULL ENABLE, 
			LAST_MODIFIED_AT TIMESTAMP (6) WITH LOCAL TIME ZONE DEFAULT ON NULL LOCALTIMESTAMP NOT NULL ENABLE, 
			LAST_MODIFIED_BY VARCHAR2(32 CHAR) DEFAULT ON NULL NVL(SYS_CONTEXT('APEX$SESSION','APP_USER'), SYS_CONTEXT('USERENV','SESSION_USER')) NOT NULL ENABLE, 
...
begin
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'DIAGRAM_EDGES';
	if v_count = 0 then 
		v_stat := q'[
		CREATE TABLE DIAGRAM_EDGES 
		(	ID NUMBER DEFAULT ON NULL DIAG_EDGES_SEQ.NEXTVAL NOT NULL ENABLE, 
