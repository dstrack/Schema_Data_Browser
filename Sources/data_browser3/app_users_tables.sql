...
$END
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_OBJECTS WHERE OBJECT_NAME = 'USER_WORKSPACE_SESSIONS';
	if v_count = 0 then 
		v_stat := q'[
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_DEPENDENCIES where NAME = 'V_CONTEXT_USERS' and REFERENCED_NAME != 'APP_USERS';
	if v_count = 0 then 
		v_stat := q'[
	...
	SELECT COUNT(*) INTO v_count
	FROM USER_DEPENDENCIES where NAME = 'VDATA_BROWSER_USERS' and REFERENCED_NAME NOT IN ('USER_NAMESPACES', 'APP_USERS');
	if v_count = 0 then 
		v_stat := q'[
		CREATE OR REPLACE VIEW VDATA_BROWSER_USERS AS
		SELECT B.WORKSPACE$_ID, B.WORKSPACE_NAME, B.WORKSPACE_STATUS, B.WORKSPACE_TYPE, B.TEMPLATE_NAME,
			B.APPLICATION_GROUP, B.APPLICATION_ID, 
			A.ID USER_ID, A.USER_LEVEL, 
			A.EMAIL_ADDRESS, A.EMAIL_VALIDATED, A.EMAIL_VALIATION_TOKEN, A.SEND_MAIL_REPLY_MESSAGE, 
			A.LOGIN_NAME, A.UPPER_LOGIN_NAME, A.LAST_LOGIN_DATE, A.FIRST_NAME, A.LAST_NAME, A.LANGUAGE_CODE, 
			A.PASSWORD_HASH, A.PASSWORD_RESET, A.PASSWORD_EXPIRATION_DATE, A.ACCOUNT_EXPIRATION_DATE, 
			A.ACCOUNT_LOCKED, A.CREATED_BY, A.CREATED_AT, A.LAST_MODIFIED_BY, A.LAST_MODIFIED_AT
		FROM APP_USERS A 
		CROSS JOIN USER_NAMESPACES B
		WHERE B.WORKSPACE_NAME = 
		]' 
		|| DBMS_ASSERT.ENQUOTE_LITERAL(SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA'));
		EXECUTE IMMEDIATE v_Stat;
	end if;
end;
/
