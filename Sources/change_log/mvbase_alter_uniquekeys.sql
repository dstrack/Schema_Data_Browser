...
CREATE MATERIALIZED VIEW MVBASE_ALTER_UNIQUEKEYS
    ...
    HAS_SCALAR_VIEW_KEY, HAS_SERIAL_VIEW_KEY, HAS_WORKSPACE_ID, HAS_DELETE_MARK, INCLUDE_TIMESTAMP, INCLUDE_DELETE_MARK, INCUDE_WORKSPACE_ID,
...
WITH MVIEW_LOGS AS (
    ...
    SELECT TABLE_NAME,
		...
		THEN 'YES' ELSE 'NO' END INCUDE_WORKSPACE_ID
	FROM (
...
SELECT T.TABLE_NAME, T.TABLESPACE_NAME, T.CONSTRAINT_NAME, T.CONSTRAINT_TYPE, T.READ_ONLY,
    ...
    CASE WHEN CONSTRAINT_TYPE IS NOT NULL THEN
        ...
        || case when HAS_WORKSPACE_ID != 'NO' AND INCUDE_WORKSPACE_ID = 'YES' then ' COMPRESS ' || NVL(PREFIX_LENGTH, 1) end
        || ' ' || T.STATUS || ' ' || VALIDATED
    END CREATE_STAT,
    ...
    CASE WHEN (HAS_DELETE_MARK != 'NO'
        ...
        AND ( KEY_HAS_WORKSPACE_ID = 'NO' -- Add Workspace ID
            AND INCUDE_WORKSPACE_ID = 'YES'
        OR KEY_HAS_WORKSPACE_ID != 'NO'
            AND INCUDE_WORKSPACE_ID = 'NO'  -- Drop Workspace ID
    ...
    CASE WHEN (HAS_DELETE_MARK != 'NO' AND INCLUDE_DELETE_MARK = 'YES'
              OR HAS_WORKSPACE_ID != 'NO' AND INCUDE_WORKSPACE_ID = 'YES')
        THEN 'YES' ELSE 'NO' END VIEW_IS_CREATED,
    DENSE_RANK() OVER (PARTITION BY T.TABLE_NAME
        ORDER BY 
        	CASE WHEN IS_REFERENCED_KEY = 'YES' THEN 0 ELSE 1 END,  -- prefer referenced keys
        	CONSTRAINT_TYPE ASC, -- prefer primary keys 
        	CASE WHEN HAS_SCALAR_VIEW_KEY = 'YES' THEN 0 ELSE 1 END,
...
FROM (
    SELECT TABLE_NAME, TABLESPACE_NAME, CONSTRAINT_NAME, CONSTRAINT_TYPE, READ_ONLY,
        CAST(RTRIM (
            ...
            AND INCUDE_WORKSPACE_ID = 'YES' -- Add Workspace ID
    ...
    FROM (
        SELECT
            ...
            D.INCLUDE_TIMESTAMP, D.INCLUDE_DELETE_MARK, D.INCUDE_WORKSPACE_ID,
