...
WITH SCALAR_KEYS AS (
	select TABLE_NAME, 
		VIEW_KEY_COLS SCALAR_KEY_COLUMN, 
        HAS_SCALAR_VIEW_KEY HAS_SCALAR_KEY,
		SEQUENCE_OWNER, SEQUENCE_NAME,
		DENSE_RANK() OVER (PARTITION BY TABLE_NAME
			ORDER BY CASE WHEN 'YES' IN (KEY_HAS_NEXTVAL, KEY_HAS_SYS_GUID) THEN 0 ELSE 1 END,
    			IS_CANDIDATE_KEY DESC, CONSTRAINT_TYPE ASC, CONSTRAINT_NAME ASC
		) POSITION
	FROM MVBASE_ALTER_UNIQUEKEYS SK
	...
	GROUP BY TABLE_NAME
), CHANGELOG_REFERENCE_TABLE AS (
	SELECT DISTINCT A.TABLE_NAME, 
		FIRST_VALUE(D.TABLE_NAME) OVER (PARTITION BY A.TABLE_NAME ORDER BY Q.RN, A.DELETE_RULE, A.CONSTRAINT_NAME, B.POSITION DESC) FOREIGN_KEY_TAB
	...
	JOIN (SELECT N.COLUMN_VALUE PATTERN, ROWNUM RN FROM TABLE( changelog_conf.in_list(changelog_conf.Get_IncludeChangeLogPattern, ',') ) N) Q
	ON changelog_conf.Get_BaseName(D.TABLE_NAME) LIKE Q.PATTERN ESCAPE '\'
	WHERE A.CONSTRAINT_TYPE = 'R'
	AND B.COLUMN_NAME <> changelog_conf.Get_ColumnWorkspace
	AND B.TABLE_NAME <> D.TABLE_NAME -- not recursive connection
	AND changelog_conf.Match_Column_Pattern(changelog_conf.Get_BaseName(D.TABLE_NAME), changelog_conf.Get_ExcludeChangeLogPattern) = 'NO'
) 		
SELECT T.*,
	...
	CASE WHEN (changelog_conf.Match_Column_Pattern(T.VIEW_NAME, changelog_conf.Get_IncludeChangeLogPattern) = 'YES'
			OR CHANGELOG_REFERENCE_TABLE IS NOT NULL)
...
FROM (
	SELECT  
		...
		PK.KEY_IS_ALTERED, PK.VIEW_IS_CREATED, 
		PK.INCLUDE_TIMESTAMP, PK.INCLUDE_DELETE_MARK, PK.INCLUDE_WORKSPACE_ID,
		...
		RT.FOREIGN_KEY_TAB CHANGELOG_REFERENCE_TABLE		
	FROM MVBASE_ALTER_UNIQUEKEYS PK, BASE_VIEWS BV, CHANGE_LOG C, SCALAR_KEYS SK, CHANGELOG_REFERENCE_TABLE RT
	WHERE PK.TABLE_NAME = BV.TABLE_NAME (+)
	AND PK.TABLE_NAME = C.TABLE_NAME (+)
	AND PK.TABLE_NAME = SK.TABLE_NAME (+) AND SK.POSITION (+) = 1
	AND PK.TABLE_NAME = RT.TABLE_NAME (+)
	AND PK.POSITION = 1
) T, TABLE_STATUS  C
...
FROM (
    ...
    FROM (
        SELECT
            ...
            P.UNIQUE_KEY_COLS,
            P.VIEW_KEY_COLS,
            CASE WHEN P.INCLUDE_WORKSPACE_ID = 'YES'
...
FROM (
	...
	FROM (
		...
		AND BV.VIEW_NAME != BV.TABLE_NAME -- Bugfix DS 20230216 Source and Traget must be views
		AND FV.VIEW_NAME != FV.TABLE_NAME
