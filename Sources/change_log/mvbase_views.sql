...
CREATE OR REPLACE VIEW VBASE_VIEWS
AS
WITH SCALAR_KEYS AS (
    ...
    FROM (
		SELECT DISTINCT TABLE_NAME,  COLUMN_ID,  COLUMN_NAME, DATA_TYPE, NULLABLE, DEFAULT_LENGTH, DEFAULT_TEXT, 
		...
		FROM (
			SELECT TABLE_NAME,  COLUMN_ID,  COLUMN_NAME, DATA_TYPE, NULLABLE, DEFAULT_LENGTH, DEFAULT_TEXT,
    ...
    SELECT DISTINCT A.TABLE_NAME, FIRST_VALUE(CAST(B.COLUMN_NAME AS VARCHAR2(128)) ) OVER (PARTITION BY A.TABLE_NAME ORDER BY Q.RN, A.DELETE_RULE, A.CONSTRAINT_NAME, B.POSITION DESC) FOREIGN_KEY_COL
      FROM USER_CONSTRAINTS A
      JOIN USER_CONS_COLUMNS B ON A.CONSTRAINT_NAME = B.CONSTRAINT_NAME     -- column of foreign key source
      JOIN USER_CONS_COLUMNS D ON A.R_CONSTRAINT_NAME = D.CONSTRAINT_NAME   -- column of foreign key target
      JOIN (SELECT N.COLUMN_VALUE TABLE_NAME, ROWNUM RN FROM TABLE( changelog_conf.in_list(changelog_conf.Get_ChangeLogFKeyTables, ',') ) N) Q
        ON changelog_conf.Get_BaseName(D.TABLE_NAME) = Q.TABLE_NAME
      WHERE A.CONSTRAINT_TYPE = 'R'
      AND B.COLUMN_NAME <> changelog_conf.Get_ColumnWorkspace
      AND B.TABLE_NAME <> D.TABLE_NAME -- not recursive connection
)
SELECT NVL(VIEW_NAME, TABLE_NAME) VIEW_NAME, TABLE_NAME, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') OWNER,
    TABLESPACE_NAME, PRIMARY_KEY_COLS, CONSTRAINT_NAME, CONSTRAINT_TYPE, 
    IS_EXTERNAL_TABLE, RUN_NO, SHORT_NAME, SHORT_NAME2,
    HAS_CREATE_TIMESTAMP, HAS_CREATE_USER, HAS_MODFIY_TIMESTAMP, HAS_MODFIY_USER, 
    HAS_SCALAR_PRIMARY_KEY, HAS_SERIAL_PRIMARY_KEY,
    ID_IS_FOREIGN_KEY, HAS_WORKSPACE_ID, HAS_DELETE_MARK, HAS_BLOB_COLUMNS, HAS_INDEX_FORMAT,
    CREATE_TIMESTAMP_COLUMN_NAME, CREATE_USER_COLUMN_NAME, MODFIY_TIMESTAMP_COLUMN_NAME, MODFIY_USER_COLUMN_NAME, INDEX_FORMAT_COLUMN_NAME,
    IS_CANDIDATE_KEY, KEY_HAS_NEXTVAL, KEY_HAS_SYS_GUID, 
	SCALAR_KEY_COLUMN, HAS_SCALAR_KEY,
    SEQUENCE_OWNER, SEQUENCE_NAME,
    KEY_IS_ALTERED, VIEW_IS_CREATED, INCLUDE_TIMESTAMP, INCLUDE_DELETE_MARK, INCUDE_WORKSPACE_ID,
    INCLUDE_CHANGELOG, 
    CHANGELOG_KEY_COL,
	CASE WHEN EXISTS (SELECT 1
			FROM (SELECT N.COLUMN_VALUE PATTERN FROM TABLE( changelog_conf.in_list(changelog_conf.Get_ExcludeTablesPattern, ',') ) N) P
			WHERE NVL(B.VIEW_NAME, B.TABLE_NAME) LIKE P.PATTERN ESCAPE '\'
		) THEN 'YES' ELSE 'NO' END EXCLUDED_TABLE,
    -- create instead of UPDATE trigger for views without changelog
    -- the current row is marked as deleted and a new row is inserted
    CASE WHEN changelog_conf.Get_Add_Update_Trigger1 = 'YES'
    		AND B.VIEW_IS_CREATED = 'YES'
    		AND HAS_MODFIY_TIMESTAMP = 'READY'
            AND B.HAS_DELETE_MARK = 'READY'
            AND B.INCLUDE_CHANGELOG = 'NO'
            AND B.HAS_SERIAL_PRIMARY_KEY = 'YES'
            AND B.INCLUDE_DATA_COLUMNS = 'YES'	-- There exists a column that is not part of the primary key or a house keeping column
            THEN CASE WHEN VIEW_UPDATE_TRIGGER_NAME IS NULL THEN 'YES' ELSE 'READY' END
            ELSE CASE WHEN VIEW_UPDATE_TRIGGER_NAME IS NULL THEN 'NO' ELSE 'DROP' END
            END ADD_UPDATE_TRIGGER1,
    CASE WHEN changelog_conf.Get_Add_Update_Trigger2 = 'YES'
    		AND B.VIEW_IS_CREATED = 'YES'
    		AND HAS_MODFIY_TIMESTAMP = 'READY'
            AND B.HAS_DELETE_MARK = 'READY'
            AND HAS_BLOB_COLUMNS = 'YES'
            AND B.HAS_SERIAL_PRIMARY_KEY = 'YES'
            AND B.INCLUDE_DATA_COLUMNS = 'YES'	-- There exists a column that is not part of the primary key or a house keeping column
            THEN CASE WHEN VIEW_UPDATE_TRIGGER_NAME IS NULL THEN 'YES' ELSE 'READY' END
            ELSE CASE WHEN VIEW_UPDATE_TRIGGER_NAME IS NULL THEN 'NO' ELSE 'DROP' END
            END ADD_UPDATE_TRIGGER2,
    -- create instead of INSERT trigger for views with composite primary keys
    -- a trigger will reactivate deleted rows with the same primary key values
    CASE WHEN changelog_conf.Get_Add_Insert_Trigger = 'YES'
    		AND B.VIEW_IS_CREATED = 'YES'
    		AND B.HAS_MODFIY_TIMESTAMP = 'READY'
            AND B.HAS_DELETE_MARK = 'READY'
            AND B.IS_CANDIDATE_KEY = 'YES'
            AND (B.HAS_SCALAR_PRIMARY_KEY = 'YES' AND B.INCLUDE_DATA_COLUMNS = 'NO'  -- There exists no column that is not part of the primary key or a house keeping column
            OR B.HAS_SERIAL_PRIMARY_KEY = 'NO' AND INSTR(B.PRIMARY_KEY_COLS, ',') > 0 )
            THEN CASE WHEN VIEW_INSERT_TRIGGER_NAME IS NULL THEN 'YES' ELSE 'READY' END
            ELSE CASE WHEN VIEW_INSERT_TRIGGER_NAME IS NULL THEN 'NO' ELSE 'DROP' END
            END ADD_INSERT_TRIGGER,
    CASE WHEN changelog_conf.Get_Add_Delete_Trigger = 'YES'
    		AND B.VIEW_IS_CREATED = 'YES'
    		AND B.IS_CANDIDATE_KEY = 'YES'        -- table has an unique key without nulls
            AND B.PRIMARY_KEY_COLS IS NOT NULL
            AND (B.HAS_DELETE_MARK = 'READY'
            OR EXISTS (
                SELECT 'X'  -- table is referenced in a foreign key clause with on delete set null clause
                FROM USER_CONSTRAINTS A
                JOIN USER_CONSTRAINTS P ON A.R_CONSTRAINT_NAME = P.CONSTRAINT_NAME
                WHERE P.TABLE_NAME = B.TABLE_NAME -- table is referenced in a foreign key clause
                AND A.CONSTRAINT_TYPE = 'R'
                AND P.CONSTRAINT_TYPE IN ('P', 'U')
                AND (A.DELETE_RULE = 'SET NULL'     -- before migration
                OR A.DELETE_RULE = 'NO ACTION'  -- after migration
                AND EXISTS (
                    SELECT 'X' -- a trigger to support the set null clause exists
            ...
            )) THEN CASE WHEN VIEW_DELETE_TRIGGER_NAME IS NULL THEN 'YES' ELSE 'READY' END
            ELSE CASE WHEN VIEW_DELETE_TRIGGER_NAME IS NULL THEN 'NO' ELSE 'DROP' END
            END ADD_DELETE_TRIGGER
FROM (
    SELECT T.VIEW_NAME, T.TABLE_NAME, TABLESPACE_NAME, PRIMARY_KEY_COLS,
        RUN_NO, SHORT_NAME, SHORT_NAME2,
        ...
        case when HAS_SERIAL_PRIMARY_KEY = 'YES' AND ID_IS_FOREIGN_KEY = 'NO'
            then 'YES' else 'NO' end HAS_SERIAL_PRIMARY_KEY,
        HAS_WORKSPACE_ID, HAS_DELETE_MARK, IS_CANDIDATE_KEY, KEY_HAS_NEXTVAL, KEY_HAS_SYS_GUID, 
		SCALAR_KEY_COLUMN, HAS_SCALAR_KEY,
        SEQUENCE_OWNER, SEQUENCE_NAME,
        ID_IS_FOREIGN_KEY,
        HAS_SCALAR_PRIMARY_KEY, KEY_IS_ALTERED, VIEW_IS_CREATED, INCLUDE_TIMESTAMP, INCLUDE_DELETE_MARK, INCUDE_WORKSPACE_ID,
        CASE WHEN changelog_conf.Match_Column_Pattern(NVL(T.VIEW_NAME, T.TABLE_NAME), changelog_conf.Get_IncludeChangeLogPattern) = 'YES'
			AND changelog_conf.Match_Column_Pattern(NVL(T.VIEW_NAME, T.TABLE_NAME), changelog_conf.Get_ExcludeChangeLogPattern) = 'NO'
            AND T.TABLE_NAME != changelog_conf.Get_ChangeLogTable
        THEN 'YES' ELSE 'NO' END INCLUDE_CHANGELOG,
        CASE WHEN EXISTS ( -- There exists a column that is not part of the primary key or a house keeping column
                SELECT 1
                FROM USER_TAB_COLUMNS C
                WHERE C.TABLE_NAME = T.TABLE_NAME
				AND changelog_conf.Match_Column_Pattern(COLUMN_NAME, changelog_conf.Get_ColumnWorkspace_List) = 'NO'
				...
				AND changelog_conf.Match_Column_Pattern(COLUMN_NAME, REPLACE(T.PRIMARY_KEY_COLS, ' ')) = 'NO'
            ) THEN 'YES' ELSE 'NO' END INCLUDE_DATA_COLUMNS,
        CHANGELOG_KEY_COL, CONSTRAINT_NAME, CONSTRAINT_TYPE,
        CASE WHEN EXISTS (
        	SELECT 1 
        	FROM USER_EXTERNAL_TABLES S
    	...
    	CREATE_TIMESTAMP_COLUMN_NAME, CREATE_USER_COLUMN_NAME, MODFIY_TIMESTAMP_COLUMN_NAME, MODFIY_USER_COLUMN_NAME, INDEX_FORMAT_COLUMN_NAME,
        R.VIEW_INSERT_TRIGGER_NAME,
        R.VIEW_UPDATE_TRIGGER_NAME,
        R.VIEW_DELETE_TRIGGER_NAME
    FROM (
        SELECT BV.VIEW_NAME, PK.TABLE_NAME, TABLESPACE_NAME,
            ...
            CASE WHEN EXISTS (SELECT 1
              FROM USER_CONSTRAINTS A
              JOIN USER_CONS_COLUMNS B ON A.CONSTRAINT_NAME = B.CONSTRAINT_NAME     -- column of foreign key source
              JOIN USER_CONS_COLUMNS P ON A.TABLE_NAME = P.TABLE_NAME AND B.COLUMN_NAME = P.COLUMN_NAME -- column of primary key
              JOIN USER_CONS_COLUMNS D ON A.R_CONSTRAINT_NAME = D.CONSTRAINT_NAME   -- column of foreign key target
              WHERE P.CONSTRAINT_NAME = PK.CONSTRAINT_NAME
              AND A.TABLE_NAME = PK.TABLE_NAME
              AND A.CONSTRAINT_TYPE = 'R'
              AND B.COLUMN_NAME <> changelog_conf.Get_ColumnWorkspace
            ) THEN 'YES' ELSE 'NO' END ID_IS_FOREIGN_KEY,
            -- when this table has a foreign key to a table that is matching the Get_IncludeChangeLogPattern
            -- then this table is also included in the set of tables with changelog
        ...
        AND PK.POSITION = 1
    ) T, TABLE_STATUS  C, VIEW_TRIGGERS R   
   WHERE T.TABLE_NAME = C.TABLE_NAME (+)
   AND T.VIEW_NAME = R.TABLE_NAME (+)
) B;
...
FROM (
    ...
    FROM (
        SELECT
            ...
            CAST(changelog_conf.F_BASE_KEY_COLS(A.TABLE_NAME, A.CONSTRAINT_NAME) AS VARCHAR2(512)) FOREIGN_KEY_COLS,
            CAST(changelog_conf.F_VIEW_KEY_COLS(A.TABLE_NAME, A.CONSTRAINT_NAME) AS VARCHAR2(512)) FOREIGN_KEY_COLS2,
            ...
            '_V' || A.CONSTRAINT_TYPE || 'K' || NULLIF(DENSE_RANK() OVER (PARTITION BY A.TABLE_NAME ORDER BY A.CONSTRAINT_NAME), 1) CONSTRAINT_EXT
        FROM USER_CONSTRAINTS A, MVBASE_ALTER_UNIQUEKEYS P, MVBASE_ALTER_UNIQUEKEYS B, BASE_VIEWS FV
...
FROM (
	...
	FROM (
		...
		FROM BASE_VIEWS BV, MVBASE_FOREIGNKEYS FK, BASE_VIEWS FV
...
WITH UNIQUE_KEYS AS ( -- find unique column expression for referenced table
    ...
    FROM (
        SELECT DISTINCT B.TABLE_NAME, B.CONSTRAINT_NAME, A.CONSTRAINT_TYPE, B.COLUMN_NAME, B.POSITION, N.COLUMN_VALUE COLUMN_PATTERN
...
DESCRIPTION_KEYS AS ( -- find columns that match a name in g_ReferenceDescriptionCols for referenced table
    ...
    FROM (
        ...
        , USER_TAB_COLUMNS D WHERE D.COLUMN_NAME LIKE N.COLUMN_PATTERN ESCAPE '\'
...
CREATE OR REPLACE VIEW VBASE_TRIGGER
    (TABLE_NAME, TRIGGER_NAME, TRIGGERING_EVENT, TRIGGER_TYPE, TRIGGER_BODY, IS_CANDIDATE)
AS
SELECT T.TABLE_NAME, T.TRIGGER_NAME, T.TRIGGERING_EVENT, T.TRIGGER_TYPE,
    T.TRIGGER_BODY,
    CASE WHEN ((T.TRIGGER_TYPE = 'BEFORE EACH ROW'
        AND ((T.TRIGGERING_EVENT = 'INSERT' AND T.TRIGGER_NAME = changelog_conf.Get_BiTrigger_Name(B.SHORT_NAME))
            OR (T.TRIGGERING_EVENT = 'UPDATE' AND T.TRIGGER_NAME = changelog_conf.Get_BuTrigger_Name(B.SHORT_NAME))
        ...
        OR (T.TRIGGER_TYPE = 'COMPOUND'  AND T.TRIGGER_NAME = changelog_conf.Get_ChangelogTrigger_Name(B.SHORT_NAME, RUN_NO)
