...
IS
	...
	IS
        ...
        IS
			SELECT T.TABLE_NAME, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') TABLE_OWNER,
					T.TRIGGER_BODY, 
					S.REFERENCED_OWNER SEQUENCE_OWNER, 
					S.REFERENCED_NAME SEQUENCE_NAME
			FROM (
	...
	IS
        ...
        IS
			...
			AND C.OWNER NOT IN ('SYS', 'SYSTEM', 'SYSAUX', 'CTXSYS', 'MDSYS');
	...
	IS
        ...
        IS
			SELECT /*+ RESULT_CACHE */
				...
				D.DATA_TYPE, D.NULLABLE, D.DATA_PRECISION, D.DATA_SCALE, D.CHAR_LENGTH,
				B.INDEX_OWNER, B.INDEX_NAME, B.DEFERRABLE, B.DEFERRED, B.STATUS, B.VALIDATED
			FROM SYS.ALL_CONSTRAINTS B, SYS.ALL_CONS_COLUMNS C, SYS.ALL_TAB_COLS D
			...
			AND B.OWNER NOT IN ('SYS', 'SYSTEM', 'SYSAUX', 'CTXSYS', 'MDSYS');
        CURSOR user_keys_cur
        IS
			SELECT /*+ RESULT_CACHE */
				C.TABLE_NAME, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') TABLE_OWNER, C.CONSTRAINT_NAME, B.CONSTRAINT_TYPE, C.COLUMN_NAME, C.POSITION,
				D.DATA_DEFAULT DEFAULT_TEXT,
				D.DATA_TYPE, D.NULLABLE, D.DATA_PRECISION, D.DATA_SCALE, D.CHAR_LENGTH,
				B.INDEX_OWNER, B.INDEX_NAME, B.DEFERRABLE, B.DEFERRED, B.STATUS, B.VALIDATED
			FROM SYS.USER_CONSTRAINTS B, SYS.USER_CONS_COLUMNS C, SYS.USER_TAB_COLS D
			WHERE C.TABLE_NAME = B.TABLE_NAME AND C.CONSTRAINT_NAME = B.CONSTRAINT_NAME 
	...
	IS
        ...
        IS
			SELECT /*+ RESULT_CACHE */
				C.TABLE_NAME, C.TABLE_OWNER, SUBSTR(C.INDEX_NAME, 1, 27) || '_IC' CONSTRAINT_NAME, 'U' CONSTRAINT_TYPE, C.COLUMN_NAME, C.COLUMN_POSITION POSITION,
				D.DATA_DEFAULT DEFAULT_TEXT,
				D.DATA_TYPE, D.NULLABLE, D.DATA_PRECISION, D.DATA_SCALE, D.CHAR_LENGTH,
			...
			AND C.TABLE_OWNER NOT IN ('SYS', 'SYSTEM', 'SYSAUX', 'CTXSYS', 'MDSYS');
        CURSOR user_keys_cur
        IS
			SELECT  /*+ RESULT_CACHE */
				C.TABLE_NAME, SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') TABLE_OWNER, 
