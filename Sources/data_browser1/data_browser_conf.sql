...
IS
	...
	TYPE rec_record_edit IS RECORD (
		...
		CHECK_CONDITION		VARCHAR2(1024),		-- Check expression for this column
		CHECK_CONSTRAINT_NAME VARCHAR2(128),
		CONSTRAINT_NAME		VARCHAR2(128),
		COLUMN_ALIGN 		VARCHAR2(10),
		COLUMN_HEADER 		VARCHAR2(128),
		COLUMN_EXPR 		VARCHAR2(32767),
...
*/
	...
	is
		...
		CURSOR Tables_cur1
		IS
			SELECT SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') OWNER, C.TABLE_NAME, C.CONSTRAINT_NAME, 
...
$END
	...
	is
		...
		IS
			...
			JOIN SYS.USER_CONS_COLUMNS B ON A.CONSTRAINT_NAME = B.CONSTRAINT_NAME AND A.TABLE_NAME = B.TABLE_NAME
			WHERE A.SEARCH_CONDITION != DBMS_ASSERT.ENQUOTE_NAME(B.COLUMN_NAME) || ' IS NOT NULL' -- filter NOT NULL checks
		;
		CURSOR Tables_cur2
		IS
			SELECT B.OWNER, B.TABLE_NAME, B.COLUMN_NAME, B.POSITION, B.CONSTRAINT_NAME, A.SEARCH_CONDITION
    ...
    BEGIN
		...
		FROM (
			...
			FROM (
				SELECT A.TABLE_NAME, A.COLUMN_NAME, A.CONSTRAINT_NAME, A.SEARCH_CONDITION
			...
			AND REGEXP_INSTR(SEARCH_CONDITION, '\WOR\W', 1, 1, 1, 'i') = 0
			AND REGEXP_INSTR(SEARCH_CONDITION, '\WAND\W', 1, 1, 1, 'i') = 0
