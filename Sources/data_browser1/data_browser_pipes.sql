...
IS
        ...
        IS
        	...
        	FROM (
				...
				FROM (
					...
					where G.TYPE = 'TABLE'
					AND A.OWNER NOT IN (SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA'), 'SYS', 'SYSTEM', 'SYSAUX', 'CTXSYS', 'MDSYS')
					AND A.IOT_NAME IS NULL	-- skip overflow tables of index organized tables
					AND A.TABLE_NAME NOT LIKE 'DR$%$_'  -- skip fulltext index
					AND A.TEMPORARY = 'N'	-- skip temporary tables
					AND A.SECONDARY = 'N'
	...
	IS
		...
		SELECT /*+ RESULT_CACHE */
			...
			DENSE_RANK() OVER (PARTITION BY A.TABLE_NAME, A.TABLE_OWNER ORDER BY MATCHING DESC, U_CONSTRAINT_NAME) RANK
		FROM (
			SELECT DISTINCT TABLE_NAME, TABLE_OWNER, COLUMN_NAME, POSITION, NULLABLE, DATA_TYPE, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, 
				U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, CONSTRAINT_TYPE, INDEX_OWNER, INDEX_NAME
			FROM (
			...
			WHERE (U_MEMBERS > 1 OR CONSTRAINT_TYPE = 'U' AND HAS_SERIAL_KEY = 'N')
			UNION ALL
			SELECT DISTINCT TABLE_NAME, TABLE_OWNER, COLUMN_NAME, POSITION, NULLABLE, DATA_TYPE, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, 
				U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, 'I' CONSTRAINT_TYPE, INDEX_OWNER, INDEX_NAME
			FROM (
	...
	IS
		...
		JOIN SYS.ALL_CONSTRAINTS C ON F.R_CONSTRAINT_NAME = C.CONSTRAINT_NAME AND F.OWNER = C.OWNER
		...
		JOIN SYS.USER_CONS_COLUMNS FC ON F.CONSTRAINT_NAME = FC.CONSTRAINT_NAME AND F.TABLE_NAME = FC.TABLE_NAME
