...
IS
	...
	IS
        CURSOR keys_cur
        IS
        	SELECT  /*+ USE_MERGE(T E R) */
        		T.TABLE_NAME, T.TABLE_OWNER, T.TABLESPACE_NAME, T.CONSTRAINT_NAME, T.CONSTRAINT_TYPE,
        	...
        	FROM (
				...
				FROM (
					select /*+ USE_MERGE(A B) */
						A.TABLE_NAME, A.OWNER TABLE_OWNER, A.TABLESPACE_NAME, 
			...
			FROM (
				...
				FROM (
					SELECT /*+ USE_MERGE(A D C) */
						D.NAME VIEW_NAME, D.OWNER VIEW_OWNER,
	...
	IS
		...
		WITH UNIQUE_KEYS AS (
			SELECT 
				...
				DENSE_RANK() OVER (PARTITION BY TABLE_NAME, TABLE_OWNER, CONSTRAINT_NAME ORDER BY POSITION) POSITION, 
				DEFAULT_TEXT, VIEW_COLUMN_NAME, COLUMN_ID, DATA_TYPE, NULLABLE, 
				DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, DEFAULT_LENGTH, VIRTUAL_COLUMN,
				INDEX_OWNER, INDEX_NAME
			FROM table (changelog_conf.FN_Pipe_unique_keys) 
			WHERE VIEW_COLUMN_NAME IS NOT NULL
			AND COLUMN_NAME NOT IN ( changelog_conf.Get_ColumnDeletedMark, changelog_conf.Get_ColumnWorkspace)
		)
		SELECT  /*+ RESULT_CACHE */
			VIEW_NAME, TABLE_NAME, TABLE_OWNER, COLUMN_NAME, 
			POSITION, NULLABLE, COLUMN_ID, DATA_TYPE, DATA_PRECISION, DATA_SCALE, 
			CHAR_LENGTH, DEFAULT_LENGTH, VIRTUAL_COLUMN,
			U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, 
			CONSTRAINT_TYPE, INDEX_OWNER, INDEX_NAME, RANK
		FROM (
			SELECT /*+ USE_MERGE(A B) */
				NVL(B.VIEW_NAME, A.TABLE_NAME) VIEW_NAME, 
				NVL(B.VIEW_OWNER, A.TABLE_OWNER) VIEW_OWNER,
				A.TABLE_NAME, A.TABLE_OWNER,
				CAST(COLUMN_NAME AS VARCHAR2(128)) COLUMN_NAME,
				POSITION, NULLABLE, COLUMN_ID, DATA_TYPE, DATA_PRECISION, DATA_SCALE, 
				CHAR_LENGTH, DEFAULT_LENGTH, VIRTUAL_COLUMN,
				U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, HAS_SCALAR_KEY,
			...
			FROM ( /*+ USE_MERGE(A B) */
				SELECT DISTINCT TABLE_NAME, TABLE_OWNER, COLUMN_NAME, 
					POSITION, NULLABLE, COLUMN_ID, DATA_TYPE, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, DEFAULT_LENGTH, VIRTUAL_COLUMN,
					U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, HAS_SCALAR_KEY, CONSTRAINT_TYPE, INDEX_OWNER, INDEX_NAME
				FROM (
					SELECT /*+ USE_MERGE(B S) */
						S.TABLE_NAME, S.TABLE_OWNER, 
						S.HAS_SCALAR_VIEW_KEY  HAS_SCALAR_KEY, 
						B.CONSTRAINT_NAME U_CONSTRAINT_NAME, B.COLUMN_NAME, B.POSITION, B.NULLABLE, 
						B.COLUMN_ID, B.DATA_TYPE, B.DATA_PRECISION, B.DATA_SCALE, B.CHAR_LENGTH, B.DEFAULT_LENGTH, B.VIRTUAL_COLUMN,
						B.CONSTRAINT_TYPE, B.INDEX_OWNER, B.INDEX_NAME, 
						COUNT(DISTINCT B.COLUMN_NAME) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) U_MEMBERS,
						COUNT(DISTINCT case when data_browser_pattern.Match_Display_Columns(B.COLUMN_NAME) = 'YES' then B.COLUMN_NAME end
                        ) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) MATCHING,
				...
				FROM (
					SELECT 
						B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME U_CONSTRAINT_NAME, B.COLUMN_NAME, 
						DENSE_RANK() OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME ORDER BY B.POSITION) POSITION, 
						B.NULLABLE, 
						B.COLUMN_ID, B.DATA_TYPE, B.DATA_PRECISION, B.DATA_SCALE, B.CHAR_LENGTH, B.DEFAULT_LENGTH, B.VIRTUAL_COLUMN,
						B.CONSTRAINT_TYPE, B.INDEX_OWNER, B.INDEX_NAME, 
						COUNT(DISTINCT B.COLUMN_NAME) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) U_MEMBERS,
					...
					WHERE COLUMN_NAME NOT IN ( changelog_conf.Get_ColumnDeletedMark, changelog_conf.Get_ColumnWorkspace)
					AND NOT EXISTS (
						SELECT 1
						FROM UNIQUE_KEYS C
		...
		WHERE (CONSTRAINT_TYPE = 'U' OR (RANK = 1 AND HAS_SCALAR_KEY = 'NO'))
		AND VIEW_OWNER = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA');
	...
	IS
		...
		SELECT 
			...
			CASE WHEN MIN(COLUMN_PREFIX) = MAX(COLUMN_PREFIX) THEN SUBSTR(MAX(COLUMN_NAME), 1, MAX(COLUMN_PREFIX)) END COLUMN_PREFIX
		FROM (
			...
			FROM (
				SELECT C.TABLE_NAME, C.TABLE_OWNER, C.COLUMN_NAME, C.COLUMN_ID,
					INSTR(C.COLUMN_NAME, '_') COLUMN_PREFIX,
