...
IS
	...
	BEGIN
		if p_Owner = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') then 
			...
			FROM (
				...
				AND A.OBJECT_NAME IN ('CHANGELOG_CONF', 'DATA_BROWSER_CONF')
				UNION ALL
	...
	BEGIN
		v_LAST_DDL_TIME := NVL(p_LAST_DDL_TIME, Get_Last_DDL_Time(p_Dependent_MViews, p_Owner));
		if p_Owner = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') then 
		...
		for v_Step in p_Start_Step..v_Steps loop
			case v_Step 
				when 1 then 
					...
					v_LAST_DDL_TIME := Get_Last_DDL_Time(null, p_Owner);
				...
				when 7 then MView_Refresh(p_MView_Name => 'MVDATA_BROWSER_DESCRIPTIONS', 
										p_Dependent_MViews => 'MVDATA_BROWSER_VIEWS,MVDATA_BROWSER_D_REFS,MVDATA_BROWSER_U_REFS', 
										p_Owner => p_Owner, p_LAST_DDL_TIME => v_LAST_DDL_TIME);
				when 8 then MView_Refresh(p_MView_Name => 'MVDATA_BROWSER_CHECKS_DEFS', 
										p_Dependent_MViews => 'MVDATA_BROWSER_VIEWS,MVDATA_BROWSER_FKEYS,MVDATA_BROWSER_DESCRIPTIONS', 
										p_Owner => p_Owner, p_LAST_DDL_TIME => v_LAST_DDL_TIME);
