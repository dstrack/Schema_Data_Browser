...
SCALAR_KEYS AS (
	select TABLE_NAME, TABLE_OWNER,
		VIEW_KEY_COLS SCALAR_KEY_COLUMN, -- Bugfix DS 20230220: filter workspace$_id column
		HAS_SCALAR_VIEW_KEY HAS_SCALAR_KEY,
	...
	WHERE SK.HAS_SCALAR_VIEW_KEY = 'YES'
)
...
FROM (
	SELECT /*+ USE_MERGE(PK S RV SK) */ 
		...
		case when RV.FOLDER_NAME_COLUMN_NAME IS NOT NULL AND RV.FOLDER_PARENT_COLUMN_NAME IS NOT NULL
			THEN RV.FOLDER_NAME_COLUMN_NAME
		end FOLDER_NAME_COLUMN_NAME,
		case when RV.FOLDER_NAME_COLUMN_NAME IS NOT NULL AND RV.FOLDER_PARENT_COLUMN_NAME IS NOT NULL
			THEN RV.FOLDER_PARENT_COLUMN_NAME
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_FKEYS (
	...
	TABLE_ALIAS, FK_COLUMN_COUNT, FK_COLUMN_ID, FK_NULLABLE, CONSTRAINT_NAME, DELETE_RULE,
...
SELECT TABLE_NAME, OWNER, VIEW_NAME, UNIQUE_KEY_COLS, FOREIGN_KEY_COLS, FK_COLUMN_NAME, NORM_COLUMN_NAME, 
	TABLE_ALIAS, FK_COLUMN_COUNT, FK_COLUMN_ID,
...
FROM (
	...
	FROM (
		SELECT DISTINCT F.TABLE_NAME, F.OWNER,
			...
			F.FOREIGN_KEY_COLS,
			F.FK_COLUMN_NAME,
			CAST(data_browser_conf.Normalize_Column_Name(p_Column_Name => F.FK_COLUMN_NAME, p_Remove_Prefix => F.COLUMN_PREFIX) AS VARCHAR2(128)) NORM_COLUMN_NAME,
			CAST(data_browser_conf.Sequence_To_Table_Alias(DENSE_RANK() OVER (PARTITION BY F.VIEW_NAME ORDER BY F.FK_COLUMN_ID)) AS VARCHAR2(10)) TABLE_ALIAS,
			F.FK_COLUMN_COUNT,
			F.FK_COLUMN_ID,
			F.FK_NULLABLE_COUNT,
		...
		FROM (
            ...
            WHERE F.COLUMN_NAME NOT IN ( changelog_conf.Get_ColumnDeletedMark, changelog_conf.Get_ColumnWorkspace)
		) F
		...
		WHERE F.FK_COLUMN_COUNT = T.KEY_COLS_COUNT -- Bugfix DS 20230220: filter foreign keys with workspace$_id columns
	) F
) WHERE FK_RANK = 1
;
...
AS
	SELECT B.VIEW_NAME, B.TABLE_NAME, B.TABLE_OWNER,
		...
		DENSE_RANK() OVER (PARTITION BY A.TABLE_NAME, B.TABLE_OWNER 
            ORDER BY case when F.COLUMN_NAME is not null then 0 else 1 end	-- give foreign keys high priority
            , case when A.NULLABLE = 'N' then 0 else 1 end -- give required columns medium priority
            , A.COLUMN_ID
        ) COL_RANK
    ...
    LEFT OUTER JOIN TABLE ( changelog_conf.FN_Pipe_Foreign_Key_Columns ) F 
        ON F.TABLE_NAME = A.TABLE_NAME AND F.TABLE_OWNER = B.TABLE_OWNER AND F.COLUMN_NAME = A.COLUMN_NAME
...
WITH TABLES_D_REFS AS (
	SELECT C.VIEW_NAME,
		LISTAGG(C.COLUMN_NAME, ', ') WITHIN GROUP (ORDER BY C.COLUMN_ID) MATCHING_COLUMN_NAMES,
    ...
    GROUP BY C.VIEW_NAME, C.MEMBERS, C.HAS_NULLABLE
), TABLES_U_REFS AS (
    SELECT C.VIEW_NAME,
		LISTAGG(C.COLUMN_NAME, ', ') WITHIN GROUP (ORDER BY C.POSITION) UNIQUE_COLUMN_NAMES,
    ...
    WHERE C.RANK = 1
    GROUP BY C.VIEW_NAME, C.U_MEMBERS, C.HAS_NULLABLE, C.U_CONSTRAINT_NAME
), TABLES_C_REFS AS (
	SELECT A.VIEW_NAME,
		...
		case when COUNT(DISTINCT A.COLUMN_NAME) = 1
   ...
   SELECT VIEW_NAME, 
        MAX(FOREIGN_KEY_COLS) KEEP (DENSE_RANK FIRST ORDER BY FK_COLUMN_ID) CONTAINER_ID
   ...
   SELECT VIEW_NAME, 
        MAX(FOREIGN_KEY_COLS) KEEP (DENSE_RANK FIRST ORDER BY FK_COLUMN_ID) FOLDER_PARENT_ID
	FROM MVDATA_BROWSER_FKEYS
    WHERE VIEW_NAME = R_VIEW_NAME 
	and FK_NULLABLE = 'Y' 
	and DELETE_RULE = 'CASCADE'
...
SELECT T.VIEW_NAME, T.VIEW_OWNER, T.TABLE_NAME, T.TABLE_OWNER, T.SHORT_NAME,
	...
	CAST(case when FOLDER_NAME_COLUMN_NAME IS NOT NULL AND FOLDER_PARENT_COLUMN_NAME IS NOT NULL
			THEN FOLDER_PARENT_COLUMN_NAME
		end AS VARCHAR2(128)) FOLDER_PARENT_COLUMN_NAME, 
	CAST(FOLDER_CONTAINER_COLUMN_NAME AS VARCHAR2(128)) FOLDER_CONTAINER_COLUMN_NAME, 
	CAST(case when FOLDER_NAME_COLUMN_NAME IS NOT NULL AND FOLDER_PARENT_COLUMN_NAME IS NOT NULL
			THEN FOLDER_NAME_COLUMN_NAME
		end AS VARCHAR2(128)) FOLDER_NAME_COLUMN_NAME,		
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_F_REFS (
	...
	COLUMN_ALIGN, FIELD_LENGTH, TABLE_ALIAS, HAS_HELP_TEXT, HAS_DEFAULT, IS_BLOB, 
	IS_PASSWORD, IS_AUDIT_COLUMN, IS_DATETIME,
...
SELECT VIEW_NAME, TABLE_NAME,
	...
	HAS_HELP_TEXT, HAS_DEFAULT, IS_BLOB, IS_PASSWORD, IS_AUDIT_COLUMN, IS_DATETIME, IS_READONLY, IS_HIDDEN, 
...
FROM (
	SELECT F.VIEW_NAME, F.TABLE_NAME, SD.PRIMARY_KEY_COLS, SD.SEARCH_KEY_COLS, SD.SHORT_NAME,
		...
		and data_browser_pattern.Match_DateTime_Columns(C.COLUMN_NAME) = 'YES'
			then 'Y' else 'N'
		end IS_DATETIME,
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_SIMPLE_COLS (
	...
	DATA_TYPE, DATA_TYPE_OWNER, DATA_PRECISION, DATA_SCALE, FORM_DATA_PRECISION, FORM_DATA_SCALE,
	DATA_DEFAULT, CHAR_LENGTH, NULLABLE, CHECK_UNIQUE,
...
SELECT
	...
	DATA_TYPE, DATA_TYPE_OWNER, DATA_PRECISION, DATA_SCALE, 
	FORM_DATA_PRECISION, FORM_DATA_SCALE,
	DATA_DEFAULT, CHAR_LENGTH, NULLABLE, CHECK_UNIQUE,
	IS_CHECKED, IS_PRIMARY_KEY, IS_SEARCH_KEY, IS_FOREIGN_KEY, IS_REFERENCE, 
	IS_BLOB, IS_PASSWORD, IS_DATETIME, IS_DATA_DEDUCTED, 
...
FROM (
	SELECT VIEW_NAME, TABLE_NAME, TABLE_OWNER, HAS_SCALAR_PRIMARY_KEY, IS_SERIAL_KEY,
		...
		DATA_TYPE, DATA_TYPE_OWNER, DATA_PRECISION, DATA_SCALE,
		data_browser_conf.Get_Default_Currency_Precision(DATA_PRECISION, IS_CURRENCY, DATA_TYPE) FORM_DATA_PRECISION, 
		data_browser_conf.Get_Default_Currency_Scale(DATA_SCALE, IS_CURRENCY, DATA_TYPE) FORM_DATA_SCALE, 
		CHAR_LENGTH, 
		NULLABLE,
		CHECK_UNIQUE,
		case when EXISTS (
...
WITH FOREIGN_KEY_PARENTS AS (
    ...
    SELECT C.VIEW_NAME, C.MEMBERS,
        LISTAGG(C.COLUMN_NAME, ', ') WITHIN GROUP (ORDER BY C.COLUMN_ID, C.POSITION) MATCHING_COLUMN_NAMES,
        MAX(C.COLUMN_NAME) KEEP (DENSE_RANK FIRST ORDER BY C.COLUMN_ID, C.POSITION) COLUMN_NAME,
		MAX(B.U_CONSTRAINT_NAME) U_CONSTRAINT_NAME
    FROM MVDATA_BROWSER_D_REFS C
    , MVDATA_BROWSER_U_REFS B 
    WHERE B.VIEW_NAME (+) = C.VIEW_NAME AND C.COLUMN_NAME = B.COLUMN_NAME (+)
    GROUP BY C.VIEW_NAME, C.MEMBERS
	...
	SELECT A.VIEW_NAME, 
        COUNT(*) MEMBERS,
        LISTAGG(A.COLUMN_NAME, ', ') WITHIN GROUP (ORDER BY A.COLUMN_ID) CHAR_COLUMN_NAMES,
        MAX(A.COLUMN_NAME) KEEP (DENSE_RANK FIRST ORDER BY A.COLUMN_ID) COLUMN_NAME
	FROM VDATA_BROWSER_DISPLAY_COLS A
	WHERE COL_RANK < 5
    GROUP BY A.VIEW_NAME
)
...
FROM (
	SELECT 
		...
		case when data_browser_conf.Has_Multiple_Workspaces = 'NO' then 
			data_browser_conf.FN_Query_Cardinality(VIEW_NAME, COLUMN_NAME ) 
		end NUM_ROWS, -- count of rows with a value in column_name.
