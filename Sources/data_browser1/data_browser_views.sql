...
SELECT VIEW_NAME, VIEW_OWNER, TABLE_NAME, TABLE_OWNER, PRIMARY_KEY_COLS, 
	...
	READ_ONLY
FROM (
	SELECT /*+ USE_MERGE(PK S RV CP SK) */ 
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_U_REFS
    ...
    ON DEMAND
AS 
-- find unique column LIST for referenced table
WITH UNIQUE_KEYS AS (
	SELECT TABLE_NAME, TABLE_OWNER, CONSTRAINT_NAME, CONSTRAINT_TYPE, COLUMN_NAME, 
	...
	select trim(COLUMN_VALUE) COLUMN_VALUE from TABLE( apex_string.split(data_browser_pattern.Get_Display_Columns_Pattern, ','))
)
...
FROM (
	SELECT F.VIEW_NAME, F.TABLE_NAME, S.PRIMARY_KEY_COLS, S.SEARCH_KEY_COLS, S.SHORT_NAME,
        ...
        case when (S.FILE_FOLDER_COLUMN_NAME = F.FOREIGN_KEY_COLS and C.COLUMN_NAME IN (D.FOLDER_NAME_COLUMN_NAME,D.FOLDER_PARENT_COLUMN_NAME))
