...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_F_REFS (
	...
	IS_READONLY, IS_HIDDEN, DISPLAY_IN_REPORT, IS_DISPLAYED_KEY_COLUMN, IS_REFERENCE,
...
FROM (
	...
	, TABLE (data_browser_pipes.FN_Pipe_Table_Columns) T -- , SYS.ALL_TAB_COLS T
	, MVDATA_BROWSER_VIEWS D
	, TABLES_D_REFS C 
	, TABLE (data_browser_pipes.FN_Pipe_Table_Columns) TC 
	...
	AND S.VIEW_NAME = T.TABLE_NAME  AND T.TABLE_OWNER = S.VIEW_OWNER -- only columns that appear in the view
	...
	AND T.TABLE_NAME = TC.TABLE_NAME (+) AND T.TABLE_OWNER = TC.TABLE_OWNER (+)  -- only use foreign keys with single column
