...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_VIEWS (
	...
	COLUMN_PREFIX, NUM_ROWS, READ_ONLY
)
	CACHE
	NOLOGGING
	STORAGE (
...
ALTER TABLE MVDATA_BROWSER_VIEWS ADD
 CONSTRAINT MVDATA_BROWSER_BASE_VIEWS_PK PRIMARY KEY (VIEW_NAME) USING INDEX;

CREATE INDEX MVDATA_BROWSER_BASE_VIEWS_IND ON MVDATA_BROWSER_VIEWS(VIEW_NAME, PRIMARY_KEY_COLS);
CREATE INDEX MVDATA_BROWSER_BASE_VIEWS_IND2 ON MVDATA_BROWSER_VIEWS(TABLE_NAME, TABLE_OWNER);
CREATE INDEX MVDATA_BROWSER_BASE_VIEWS_IND3 ON MVDATA_BROWSER_VIEWS(CONSTRAINT_NAME, HAS_SCALAR_PRIMARY_KEY);
...
SELECT DISTINCT S.VIEW_NAME, S.TABLE_NAME, D.COLUMN_NAME, D.COLUMN_ID,
	...
	D.DATA_TYPE, D.DATA_PRECISION, D.NULLABLE, D.DATA_SCALE, D.CHAR_LENGTH,
...
SELECT VIEW_NAME, TABLE_NAME, TABLE_OWNER,
	...
	DENSE_RANK() OVER (PARTITION BY TABLE_NAME, TABLE_OWNER ORDER BY MATCHING DESC, U_CONSTRAINT_NAME) RANK
FROM (
	SELECT DISTINCT VIEW_NAME, TABLE_NAME, TABLE_OWNER, COLUMN_NAME, POSITION, NULLABLE, DATA_TYPE, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, 
	...
	FROM (
		SELECT S.VIEW_NAME, S.TABLE_NAME, S.TABLE_OWNER, 
			case when S.SEQUENCE_NAME IS NOT NULL then S.SCALAR_KEY_COLUMN else '--' end SCALAR_KEY_COLUMN, 
		...
		FROM UNIQUE_KEYS B, MVDATA_BROWSER_VIEWS S
			, TABLE( data_browser_conf.in_list(data_browser_pattern.Get_Display_Columns_Pattern, ',')) N -- used for ranking multiple choices per table
		WHERE S.TABLE_NAME = B.TABLE_NAME AND S.TABLE_OWNER = B.TABLE_OWNER
		AND B.COLUMN_NAME LIKE N.COLUMN_VALUE (+) ESCAPE '\'
	) B  -- used columns of the primary key only for composed keys
	WHERE (U_MEMBERS > 1 OR CONSTRAINT_TYPE = 'U' AND COLUMN_NAME != SCALAR_KEY_COLUMN)
	UNION ALL
	SELECT DISTINCT VIEW_NAME, TABLE_NAME, TABLE_OWNER, COLUMN_NAME, POSITION, NULLABLE, DATA_TYPE, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, 
		U_CONSTRAINT_NAME, U_MEMBERS, MATCHING, HAS_NULLABLE, 'I' CONSTRAINT_TYPE, INDEX_OWNER, INDEX_NAME
	FROM (
		SELECT S.VIEW_NAME, S.TABLE_NAME, S.TABLE_OWNER, B.CONSTRAINT_NAME U_CONSTRAINT_NAME, B.COLUMN_NAME, B.POSITION, B.NULLABLE, B.DATA_TYPE, 
            B.DATA_PRECISION, B.DATA_SCALE, B.CHAR_LENGTH,
			N.COLUMN_VALUE COLUMN_PATTERN, B.CONSTRAINT_TYPE, B.INDEX_OWNER, B.INDEX_NAME, 
			COUNT(DISTINCT B.COLUMN_NAME) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) U_MEMBERS,
			COUNT(DISTINCT N.COLUMN_VALUE) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) MATCHING,
			SUM(case when B.NULLABLE = 'Y' then 1 else 0 end) OVER (PARTITION BY B.TABLE_NAME, B.TABLE_OWNER, B.CONSTRAINT_NAME) HAS_NULLABLE
		FROM table (changelog_conf.FN_Pipe_unique_indexes) B, MVDATA_BROWSER_VIEWS S
			, TABLE( data_browser_conf.in_list(data_browser_pattern.Get_Display_Columns_Pattern, ',')) N -- used for ranking multiple choices per table
		WHERE S.TABLE_NAME = B.TABLE_NAME AND S.TABLE_OWNER = B.TABLE_OWNER
		AND B.COLUMN_NAME LIKE N.COLUMN_VALUE (+) ESCAPE '\'
        AND NOT EXISTS (
            ...
            AND C.INDEX_OWNER = S.TABLE_OWNER
        )
	) B
);
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_FKEYS (
	TABLE_NAME, OWNER, VIEW_NAME, UNIQUE_KEY_COLS, FOREIGN_KEY_COLS, FK_COLUMN_COUNT, FK_COLUMN_ID, FK_NULLABLE, CONSTRAINT_NAME, DELETE_RULE,
...
AS
WITH CONS_COLS AS (
	SELECT TABLE_OWNER OWNER, CONSTRAINT_NAME, TABLE_NAME, 
		VIEW_KEY_COLS KEY_COLUMNS, CONSTRAINT_TYPE
	FROM MVBASE_UNIQUE_KEYS PC
)
SELECT TABLE_NAME, OWNER, VIEW_NAME, UNIQUE_KEY_COLS, FOREIGN_KEY_COLS, FK_COLUMN_COUNT, FK_COLUMN_ID,
...
FROM (
	...
	FROM (
		SELECT DISTINCT F.TABLE_NAME, F.OWNER,
			...
			CAST(KEY_COLUMNS AS VARCHAR2(512)) R_PRIMARY_KEY_COLS,
			case when data_browser_pattern.Match_Admin_Tables(T.VIEW_NAME) = 'YES'
				then 'Y' ELSE 'N'
			end R_IS_ADMIN_TABLE
		FROM (
			SELECT F.TABLE_NAME, F.OWNER,
				S.VIEW_NAME,
				S.SEARCH_KEY_COLS UNIQUE_KEY_COLS,
				CAST(LISTAGG(FC.COLUMN_NAME, ', ') WITHIN GROUP (ORDER BY FC.POSITION) OVER (PARTITION BY F.TABLE_NAME, F.OWNER, F.CONSTRAINT_NAME) AS VARCHAR2(512) ) FOREIGN_KEY_COLS,
				COUNT(DISTINCT FC.COLUMN_NAME)  OVER (PARTITION BY F.TABLE_NAME, F.OWNER, F.CONSTRAINT_NAME) FK_COLUMN_COUNT,
				FIRST_VALUE(SC.COLUMN_ID) OVER (PARTITION BY F.TABLE_NAME, F.OWNER, F.CONSTRAINT_NAME ORDER BY FC.POSITION) FK_COLUMN_ID,
				SUM(case when SC.NULLABLE = 'Y' then 1 else 0 end) OVER (PARTITION BY F.TABLE_NAME, F.OWNER, F.CONSTRAINT_NAME) FK_NULLABLE_COUNT,
				F.CONSTRAINT_NAME, F.DELETE_RULE, 
				F.R_OWNER, F.R_CONSTRAINT_NAME
			FROM MVDATA_BROWSER_VIEWS S
            JOIN SYS.ALL_CONSTRAINTS F ON S.TABLE_NAME = F.TABLE_NAME AND S.TABLE_OWNER = F.OWNER AND F.CONSTRAINT_TYPE = 'R'
			JOIN SYS.ALL_CONS_COLUMNS FC ON F.OWNER = FC.OWNER AND F.CONSTRAINT_NAME = FC.CONSTRAINT_NAME AND F.TABLE_NAME = FC.TABLE_NAME
			JOIN SYS.ALL_TAB_COLS SC ON SC.TABLE_NAME = S.VIEW_NAME AND SC.COLUMN_NAME = FC.COLUMN_NAME AND SC.OWNER = S.VIEW_OWNER AND SC.HIDDEN_COLUMN = 'NO' -- only columns that appear in the view
		) F
		JOIN CONS_COLS PC ON F.R_OWNER = PC.OWNER AND F.R_CONSTRAINT_NAME = PC.CONSTRAINT_NAME 
		JOIN MVDATA_BROWSER_VIEWS T ON T.TABLE_NAME = PC.TABLE_NAME AND T.TABLE_OWNER = PC.OWNER
	) F
) WHERE FK_RANK = 1
;
...
CREATE INDEX MVDATA_BROWSER_FOREIGNKEYS_IND ON MVDATA_BROWSER_FKEYS(R_VIEW_NAME);

CREATE OR REPLACE VIEW VUSER_FOREIGN_KEY_PARENTS
AS
...
;
COMMENT ON TABLE VUSER_FOREIGN_KEY_PARENTS IS
'For each foreign key column of a table x a list of candidate parent keys source columns from the same table x is calculated. Used to restrict LOV Lists';
...
AS
	SELECT B.VIEW_NAME, B.TABLE_NAME, B.TABLE_OWNER,
		...
		A.DATA_TYPE, A.DATA_PRECISION, A.CHAR_LENGTH, A.DATA_SCALE, A.VIRTUAL_COLUMN,
		B.ORDERING_COLUMN_NAME,
		DENSE_RANK() OVER (PARTITION BY A.TABLE_NAME ORDER BY A.COLUMN_ID) COL_RANK
	FROM MVDATA_BROWSER_VIEWS B
	JOIN SYS.ALL_TAB_COLS A ON B.VIEW_NAME = A.TABLE_NAME AND A.OWNER = B.VIEW_OWNER -- only columns that appear in the view
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_DESCRIPTIONS (
	VIEW_NAME, TABLE_NAME, TABLE_OWNER, HAS_SCALAR_PRIMARY_KEY, PRIMARY_KEY_COLS, SEARCH_KEY_COLS,
...
WITH TABLES_D_REFS AS (
    ...
    WHERE C.RANK = 1
    AND NOT EXISTS (	-- avoid display of internal primary keys
    	SELECT 1 
    	FROM MVDATA_BROWSER_VIEWS T
    	WHERE T.HAS_SCALAR_PRIMARY_KEY = 'YES'
    	AND T.CONSTRAINT_NAME = C.U_CONSTRAINT_NAME
...
SELECT T.VIEW_NAME, T.TABLE_NAME, T.TABLE_OWNER, HAS_SCALAR_PRIMARY_KEY, PRIMARY_KEY_COLS, SEARCH_KEY_COLS,
...
FROM (
	SELECT T.VIEW_NAME, T.TABLE_NAME, T.TABLE_OWNER, T.HAS_SCALAR_PRIMARY_KEY, 
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_F_REFS (
	...
	PRIMARY_KEY_COLS, SEARCH_KEY_COLS, SHORT_NAME, FOREIGN_KEY_COLS, FK_COLUMN_ID, R_PRIMARY_KEY_COLS, R_CONSTRAINT_TYPE,
	R_VIEW_NAME, R_TABLE_NAME, COLUMN_ID, NULLABLE, R_COLUMN_ID, POSITION, 
	R_COLUMN_NAME, R_NULLABLE, R_DATA_TYPE, R_DATA_PRECISION, R_DATA_SCALE, R_CHAR_LENGTH, 
	COLUMN_ALIGN, TABLE_ALIAS, HAS_HELP_TEXT, HAS_DEFAULT, IS_BLOB, IS_PASSWORD, IS_AUDIT_COLUMN,
	IS_READONLY, IS_HIDDEN, DISPLAY_IN_REPORT, IS_DISPLAYED_KEY_COLUMN,
...
WITH TABLES_D_REFS AS (
	SELECT VIEW_NAME, TABLE_NAME, COLUMN_NAME, COLUMN_ID,
			NULLABLE, DATA_TYPE, DATA_PRECISION, CHAR_LENGTH, DATA_SCALE,
			HAS_NULLABLE, U_CONSTRAINT_NAME,
			U_MEMBERS, U_MATCHING,
			DENSE_RANK() OVER (PARTITION BY VIEW_NAME ORDER BY RANK) RANK
	FROM (
		SELECT VIEW_NAME, TABLE_NAME, COLUMN_NAME, POSITION COLUMN_ID,
			NULLABLE, DATA_TYPE, DATA_PRECISION, CHAR_LENGTH, DATA_SCALE,
			HAS_NULLABLE, U_CONSTRAINT_NAME,
...
SELECT VIEW_NAME, TABLE_NAME,
	...
	PRIMARY_KEY_COLS,
	SEARCH_KEY_COLS,
	SHORT_NAME, FOREIGN_KEY_COLS, FK_COLUMN_ID, R_PRIMARY_KEY_COLS, R_CONSTRAINT_TYPE,
	R_VIEW_NAME, R_TABLE_NAME, COLUMN_ID, NULLABLE, R_COLUMN_ID, POSITION,
	CAST(R_COLUMN_NAME AS VARCHAR2(128)) R_COLUMN_NAME,
	R_NULLABLE, R_DATA_TYPE, R_DATA_PRECISION,
	R_DATA_SCALE, R_CHAR_LENGTH, 
	...
	IS_DISPLAYED_KEY_COLUMN, HAS_NULLABLE, U_CONSTRAINT_NAME, U_MEMBERS, U_MATCHING
FROM (
	SELECT F.VIEW_NAME, F.TABLE_NAME, S.PRIMARY_KEY_COLS, S.SEARCH_KEY_COLS, S.SHORT_NAME,
        F.FOREIGN_KEY_COLS, F.FK_COLUMN_ID, F.R_PRIMARY_KEY_COLS, F.R_CONSTRAINT_TYPE,
		...
		C.COLUMN_ID*10000+NVL(TC.COLUMN_ID,0)*100 POSITION,
		...
		CAST(case when C.COLUMN_NAME IS NOT NULL then
				case when U_MEMBERS = 1 then -- simple column name when only one member is displayed
					...
					data_browser_conf.Compose_Column_Name(
						NVL(data_browser_conf.Normalize_Column_Name(p_Column_Name => T.COLUMN_NAME, p_Remove_Prefix => S.COLUMN_PREFIX),
							data_browser_conf.Normalize_Table_Name(p_Table_Name => F.R_VIEW_NAME))
						, C.COLUMN_NAME
						, 'NO', 128)
				end
		...
		case when exists (
			...
			where CC.TABLE_NAME = S.TABLE_NAME
			and CC.COLUMN_NAME = T.COLUMN_NAME
			and CC.COMMENTS IS NOT NULL
		) then 'Y' else 'N'
		end HAS_HELP_TEXT,
		case when TC.DEFAULT_LENGTH > 0
			then 'Y' else 'N'
		end HAS_DEFAULT,
		case when (TC.DATA_TYPE IN ('BLOB', 'CLOB', 'LONG', 'ORDIMAGE')
	...
	FROM MVDATA_BROWSER_VIEWS S
    , MVDATA_BROWSER_DESCRIPTIONS SD
	, MVDATA_BROWSER_FKEYS F
	, SYS.ALL_TAB_COLS T
	, MVDATA_BROWSER_VIEWS D
	, TABLES_D_REFS C 
	, SYS.ALL_TAB_COLS TC 
	WHERE SD.VIEW_NAME = S.VIEW_NAME
	AND S.VIEW_NAME = F.VIEW_NAME AND S.TABLE_OWNER = F.OWNER  -- only use foreign keys with single column
	AND S.VIEW_NAME = T.TABLE_NAME  AND T.OWNER = S.VIEW_OWNER -- only columns that appear in the view
	-- AND F.FOREIGN_KEY_COLS = T.COLUMN_NAME 
	AND F.FK_COLUMN_ID = T.COLUMN_ID -- support for composite keys
	AND T.HIDDEN_COLUMN = 'NO'
	AND D.VIEW_NAME = F.R_VIEW_NAME AND D.VIEW_OWNER = S.VIEW_OWNER
	AND F.R_VIEW_NAME = C.VIEW_NAME (+) AND C.RANK (+) = 1 -- description for each foreign key
	AND T.TABLE_NAME = TC.TABLE_NAME (+) AND T.OWNER = TC.OWNER (+)  -- only use foreign keys with single column
	AND C.COLUMN_NAME = TC.COLUMN_NAME (+) AND TC.HIDDEN_COLUMN (+) = 'NO'
);
...
-- foreign keys with unique columns and second level foreign keys
CREATE MATERIALIZED VIEW MVDATA_BROWSER_Q_REFS
	CACHE
	NOLOGGING
	STORAGE (
	  INITIAL 1024
...
SELECT VIEW_NAME, TABLE_NAME,
	...
	IS_AUDIT_COLUMN, IS_READONLY, DISPLAY_IN_REPORT, IS_DISPLAYED_KEY_COLUMN,
	HAS_NULLABLE, U_CONSTRAINT_NAME, U_MEMBERS, U_MATCHING,
	J_VIEW_NAME,
...
ALTER TABLE MVDATA_BROWSER_Q_REFS ADD CONSTRAINT MVDATA_BROWSER_TAB_Q_REFS_PK PRIMARY KEY (VIEW_NAME, IMP_COLUMN_NAME);
COMMENT ON MATERIALIZED VIEW MVDATA_BROWSER_Q_REFS IS
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_CHECKS_DEFS (
	...
	CONS_COLS_COUNT, IS_FOREIGN_KEY, CHECK_UNIQUE, CONSTRAINT_NAME, REQUIRED, DATA_DEFAULT,
...
SELECT
	...
	CONS_COLS_COUNT, IS_FOREIGN_KEY, 
...
FROM (
	SELECT S.TABLE_OWNER, S.TABLE_NAME, S.VIEW_NAME, S.COLUMN_NAME, 
		...
		S.CHECK_CONDITION,
		S.CONS_COLS_COUNT, S.IS_FOREIGN_KEY,
		S.CHECK_UNIQUE,
		NVL(S.CONSTRAINT_NAME, S.CHECK_CONSTRAINT_NAME) CONSTRAINT_NAME,
		S.REQUIRED,
		S.DATA_DEFAULT,
...
FROM ( -- remove quotes
   SELECT TABLE_NAME, TABLE_OWNER, COLUMN_NAME, REGEXP_REPLACE(COLUMN_VALUE, '^''(.*)''$', '\1') COLUMN_VALUE,
    ...
    FROM ( -- convert values to rows
		...
		AND REGEXP_INSTR(SEARCH_CONDITION_VC, '\WOR\W', 1, 1, 1, 'i') = 0
        AND REGEXP_INSTR(SEARCH_CONDITION_VC, '\WAND\W', 1, 1, 1, 'i') = 0
    ) S,
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_SIMPLE_COLS (
	...
	COLUMN_NAME, COLUMN_HEADER, COLUMN_ID, POSITION, COLUMN_ALIGN, FIELD_LENGTH,
	DATA_TYPE, DATA_TYPE_OWNER, DATA_PRECISION, DATA_SCALE, CHAR_LENGTH, NULLABLE, CHECK_UNIQUE,
	IS_CHECKED, IS_PRIMARY_KEY, IS_SEARCH_KEY, IS_FOREIGN_KEY, IS_BLOB, IS_PASSWORD, IS_DATETIME, IS_DATA_DEDUCTED, 
...
SELECT
	...
	CAST(data_browser_conf.Column_Name_to_Header(
		...
		p_Remove_Extension => case when FOREIGN_KEY_COLS IS NOT NULL -- has LOV query
			then 'YES' else 'NO' end, 
		p_Remove_Prefix => COLUMN_PREFIX, 
		p_Is_Upper_Name => IS_UPPER_NAME
	) AS VARCHAR2(128)) COLUMN_HEADER,
	COLUMN_ID, POSITION, 
	case when (FOREIGN_KEY_COLS IS NOT NULL or IS_NUMBER_YES_NO_COLUMN = 'Y' or IS_CHAR_YES_NO_COLUMN = 'Y') then 'LEFT'
...
FROM (
	SELECT VIEW_NAME, TABLE_NAME, TABLE_OWNER, HAS_SCALAR_PRIMARY_KEY, IS_SERIAL_KEY,
		COLUMN_NAME, COLUMN_ID, POSITION, FOREIGN_KEY_COLS,
		...
		case when EXISTS (
				SELECT 'X'
				FROM  MVDATA_BROWSER_CHECKS_DEFS K
				WHERE K.TABLE_NAME = T.TABLE_NAME
				AND K.TABLE_OWNER = T.TABLE_OWNER
				AND K.COLUMN_NAME = T.COLUMN_NAME
				AND CHECK_UNIQUE = 'Y'
				AND CONS_COLS_COUNT = 1
			)
			then 'Y' else 'N'
		end CHECK_UNIQUE,
		case when EXISTS (
		...
		case when FOREIGN_KEY_COLS IS NOT NULL then 'Y' else 'N' end IS_FOREIGN_KEY,
		...
		case when EXISTS (
				SELECT 'X'
				FROM TABLE( apex_string.split(T.DISPLAYED_COLUMN_NAMES, ', ') ) P
				WHERE P.COLUMN_VALUE = T.COLUMN_NAME
			)  then 'Y' else 'N'
		end IS_DISPLAYED_KEY_COLUMN,
	...
	FROM (
		SELECT S.VIEW_NAME, S.TABLE_NAME, S.TABLE_OWNER, 
			...
			T.DATA_TYPE, T.DATA_TYPE_OWNER, T.DATA_PRECISION, T.DATA_SCALE,
			T.CHAR_LENGTH, T.NULLABLE, T.DEFAULT_LENGTH, T.VIRTUAL_COLUMN,
			F.FOREIGN_KEY_COLS,
			case when T.DATA_TYPE IN ('CHAR', 'VARCHAR', 'VARCHAR2', 'NUMBER') and F.FOREIGN_KEY_COLS IS NULL then 
		...
		, MVDATA_BROWSER_FKEYS F 
        , (
			SELECT B.OWNER, B.TABLE_NAME, B.COLUMN_NAME, B.CONSTRAINT_NAME, B.SEARCH_CONDITION,
		...
		AND S.VIEW_NAME = F.VIEW_NAME (+)
		AND T.COLUMN_NAME = F.FOREIGN_KEY_COLS (+)  -- rethink the case when multiple foreign keys exist!!
		AND T.COLUMN_ID = F.FK_COLUMN_ID (+) 		-- support for composite keys
		AND S.TABLE_OWNER = F.OWNER (+)
		AND T.TABLE_NAME = B.TABLE_NAME (+)
		AND S.TABLE_OWNER = B.OWNER (+)
...
CREATE MATERIALIZED VIEW MVDATA_BROWSER_REFERENCES (
	...
	FOLDER_NAME_COLUMN_NAME, FOLDER_PARENT_COLUMN_NAME,
...
WITH TABLES_D_REFS AS ( -- find columns that match a name in g_ReferenceDescriptionCols for referenced table
    SELECT DISTINCT C.VIEW_NAME,
	...
	SELECT DISTINCT A.VIEW_NAME, A.ORDERING_COLUMN_NAME,
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM MVDATA_BROWSER_FKEYS F, MVDATA_BROWSER_VIEWS S
			WHERE F.R_VIEW_NAME = S.VIEW_NAME AND S.TABLE_OWNER = F.OWNER
		) F
...
CREATE OR REPLACE VIEW VDATA_BROWSER_JOBS_CACHED (
	SID, SERIAL#, MESSAGE, SOFAR, TOTALWORK, CONTEXT, START_TIME, LAST_UPDATE_TIME, 
	TIME_REMAINING, ELAPSED_TIME, PERCENT
)
AS
SELECT  N001 SID, N002 SERIAL#, CAST(C001 AS VARCHAR2(512)) MESSAGE, N003 SOFAR, N004 TOTALWORK, N005 CONTEXT,
...
from (
    ...
    from MVDATA_BROWSER_SIMPLE_COLS where view_name = 'DIAGRAM_EDGES'
) where view_name = 'DIAGRAM_EDGES'
