...
SELECT T.TABLE_NAME, 
        ...
        S.HAS_SCALAR_KEY, 
        S.SEARCH_KEY_COLS,
        T.D_VIEW_NAME, T.D_REF, 
    	T.R_VIEW_NAME, 
        LISTAGG(INITCAP(T.R_COLUMN_NAME), ', ') WITHIN GROUP (ORDER BY R_COLUMN_ID) COLUMN_LOOKUP_LIST,
    	D.DEFAULTS_MISSING, T.TABLE_ALIAS, 
        T.COLUMN_CNT, T.HAS_NULLABLE, T.HAS_SIMPLE_UNIQUE, 
        T.NULLABLE, -- when NULLABLE no relieable unique search 
        ...
        T.COLUMN_ID POSITION, T.POSITION2
	FROM
	(
		-- 2. level foreign keys
		SELECT Q.VIEW_NAME, Q.TABLE_NAME, Q.SEARCH_KEY_COLS, Q.SHORT_NAME,
			...
			Q.R_COLUMN_ID, Q.R_NULLABLE, Q.R_DATA_TYPE,
			Q.R_DATA_SCALE, Q.R_CHAR_LENGTH,
            Q.JOIN_CLAUSE,
			COUNT(DISTINCT Q.R_COLUMN_NAME) OVER (PARTITION BY Q.TABLE_NAME, Q.FOREIGN_KEY_COLS) COLUMN_CNT, -- columns used for lookup
		...
		FROM MVDATA_BROWSER_Q_REFS Q									-- source (columns of referenced table for lookup S_REF)
		JOIN MVDATA_BROWSER_F_REFS S ON Q.VIEW_NAME = S.VIEW_NAME		-- dest (assigned reference column D_REF)
		...
		SELECT S.VIEW_NAME,S.TABLE_NAME, S.SEARCH_KEY_COLS, S.SHORT_NAME,
			S.FOREIGN_KEY_COLS 	COLUMN_NAME,
			S.R_VIEW_NAME 		S_VIEW_NAME,
			S.IMP_COLUMN_NAME 	S_REF,
			S.TABLE_ALIAS, 
			S.R_COLUMN_NAME, 
            S.VIEW_NAME 		D_VIEW_NAME,
			S.FOREIGN_KEY_COLS 	D_REF,
			S.R_PRIMARY_KEY_COLS, S.R_CONSTRAINT_TYPE,
			S.R_VIEW_NAME, S.COLUMN_ID, S.NULLABLE,
			S.R_COLUMN_ID, S.R_NULLABLE, S.R_DATA_TYPE,
			S.R_DATA_SCALE, S.R_CHAR_LENGTH,
			NULL JOIN_CLAUSE,
            COUNT(DISTINCT S.R_COLUMN_NAME) OVER (PARTITION BY S.TABLE_NAME, S.FOREIGN_KEY_COLS) COLUMN_CNT, -- columns used for lookup
    ...
    GROUP BY T.TABLE_NAME, T.VIEW_NAME, S.HAS_SCALAR_KEY, S.SEARCH_KEY_COLS,
...
CREATE OR REPLACE VIEW VDATA_BROWSER_IMPORT_COLS
AS 
select 
	C001 column_position, 
	C002 column_name, 
	C003 data_type, 
	C004 format_mask
  from apex_collections 
where COLLECTION_NAME = data_browser_conf.Get_Import_Desc_Collection || V('APP_PAGE_ID');
