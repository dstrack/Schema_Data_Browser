...
$END
        	...
        	FROM (
				...
				FROM (
					SELECT --+ USE_NL_WITH_INDEX(D)
						...
						B.IS_REFERENCE, B.IS_SEARCHABLE_REF, B.IS_SUMMAND, B.IS_VIRTUAL_COLUMN, B.IS_DATETIME,
						...
						SUM(case when B.COLUMN_EXPR_TYPE IN ('DISPLAY_ONLY', 'LINK', 'LINK_LIST', 'LINK_ID', 'ROW_SELECTOR', 'FILE_BROWSER') then 0
							...
							OVER (ORDER BY COLUMN_ORDER, IS_AUDIT_COLUMN, COLUMN_ID, POSITION, R_VIEW_NAME RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
						)
	...
	begin
    	...
    	v_Describe_Edit_Cols_md5 := wwv_flow_item.md5 (p_Table_name, p_Unique_Key_Column, p_Data_Columns_Only, p_Select_Columns, p_Columns_Limit,
    											...
    											p_Detail_Page_ID, p_Detail_Parameter);
    ...
    IS
		...
		, PARENT_LOOKUP_Q AS (
			...
			FROM (
				...
				WHERE Q.VIEW_NAME = p_Table_name
			) S
		...
		(
			...
			FROM (
				...
				SELECT --+ INDEX(Q) USE_NL_WITH_INDEX(S)
					...
					Q.R_COLUMN_ID, Q.POSITION, Q.R_NULLABLE, Q.R_DATA_TYPE,
					Q.R_DATA_PRECISION, Q.R_DATA_SCALE, Q.R_CHAR_LENGTH,
					Q.IMP_COLUMN_NAME,
					case when p_As_Of_Timestamp = 'YES'
						then REPLACE (Q.JOIN_CLAUSE, Q.JOIN_VIEW_NAME, data_browser_conf.Get_History_View_Name(Q.JOIN_VIEW_NAME))
						else Q.JOIN_CLAUSE
					end JOIN_CLAUSE,
				...
				SELECT --+ INDEX(S)
					S.TABLE_NAME, S.VIEW_NAME, S.SEARCH_KEY_COLS, S.SHORT_NAME,
					S.FOREIGN_KEY_COLS 	COLUMN_NAME,
					S.R_VIEW_NAME 		S_VIEW_NAME,
					S.IMP_COLUMN_NAME 	S_REF,
					S.TABLE_ALIAS		TABLE_ALIAS, 
					S.R_COLUMN_NAME, 
					S.VIEW_NAME         D_VIEW_NAME,					-- lookup ids of imported table + related
					S.FOREIGN_KEY_COLS  D_REF,							-- lookup ids of imported table + related
					S.IS_FILE_FOLDER_REF,
					...
					S.R_COLUMN_ID, S.POSITION, S.R_NULLABLE, S.R_DATA_TYPE,
					S.R_DATA_PRECISION, S.R_DATA_SCALE, S.R_CHAR_LENGTH,
					S.IMP_COLUMN_NAME, NULL JOIN_CLAUSE,
