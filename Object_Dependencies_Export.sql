...
AS
WITH PARAM AS ( 
    SELECT  COALESCE(V('P28_OBJECT_TYPES'), 'CLUSTER:CONSUMER GROUP:CONTEXT:DESTINATION:DIRECTORY:EDITION:EVALUATION CONTEXT:FUNCTION:JOB:JOB CLASS:LIBRARY:LOB:LOB PARTITION:MATERIALIZED VIEW:OPERATOR:PACKAGE:PACKAGE BODY:PROCEDURE:PROGRAM:QUEUE:RESOURCE PLAN:RULE:RULE SET:SCHEDULE:SCHEDULER GROUP:TABLE:TABLE PARTITION:TABLE SUBPARTITION:TRIGGER:TYPE:TYPE BODY:UNDEFINED:UNIFIED AUDIT POLICY:VIEW:WINDOW:XML SCHEMA',
    			'CONSTRAINT:FUNCTION:INDEX:MATERIALIZED VIEW:PACKAGE:PACKAGE BODY:PROCEDURE:SEQUENCE:SYNONYM:TABLE:TRIGGER:TYPE:TYPE BODY:VIEW') Object_Types,
        V('P28_EXCLUDE_PATTERN')   Exclude_Pattern, 
